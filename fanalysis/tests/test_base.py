    ...
    def _row_topandas_comparison(self, n_components=None, row_labels=False):
        ...
        if row_labels == False:
            ...
            labels = np.loadtxt("fanalysis/tests/ca_row_labels.txt",
                                delimiter=" ", dtype=str)
            base = Base(n_components=n_components, row_labels=labels)
        df_Y = base.fit(M).row_topandas()
        Y = df_Y.as_matrix()
        df_Y_index = df_Y.index.values
        Y_row_coord_temp = Y[:, :base.n_components_]
        ...
        Y_row_coord = self._compute_Y(X_row_coord, Y_row_coord_temp,
                                      "row_coord_")
        Y[:, :base.n_components_] = Y_row_coord
        assert_array_almost_equal(X, Y)
